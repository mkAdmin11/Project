...
Устанавливается Bind9 на первый сервер, настраиваются зоны:
+ Defaul зона.
...
Устанавливается PostgreSQL. Обеспечивает работу:
+ WordPress.
+ Rouncube.
...
Установка nginx, Apache2, PHP и потребующиея в дальнейшем библиотеки. Первоначальная настройка:
+ nginx - работает как прокси.
...
Установка Certbot и получение сертификатов через webroot.

<br/> Настройка nginx, один конфиг два Server:
...
<br/> Настройка Apache2. Один конфиг, два VirtualHost для домена первого и второго уровня.
