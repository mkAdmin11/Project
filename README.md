...
> Logstash к тому времени должен быть запущен.
...
+ Слушается только сеть VPN, подключаются агенты только через VPN сеть.

*Сервер 2*
<br/> Установка zabbix-frontend-php:
+ Настроен доступ к базе даных на *Сервере 3*.
+ Настроен доступ к Zabbix-server на *Сервере 1*.
+ В конфиге Apache2 уже настроен алиас для обработки PHP фронтэнда Zabbix-а.

***Все сервера***
<br/>
Установка Zabbix-agent:
+ Настроен доступ к Zabbix-server по сети VPN.
