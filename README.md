...
+ *VPN* cеть работает поверх локальнойсети.
+ Жестко связаны *IP* адреса клиентов с их конфигами.
> Предполагается что инфраструктура выдачи ключей уже имеется и готовы пары ключей для всех серверов, а также ca и dh.
...
> Первоначальные роля хранятся в дирректории *old*.
...
> Не *localhost*, т.к. в дальнейшем *Grafana* будет обращаться к *Zabbix* напрямую через *Apache2* с *Сервера 2*.  
